...
class Dinosaur(NeuralNetwork):
    ...
    def resetStatus(self):
        ...
        self.last_action = "RUN"  # Rastrear última acción
        self.duck_timer = 0       # Contador para mantener agachado
    ...
    def update(self, userInput):
        ...
        self.last_action = userInput if isinstance(userInput, str) else self.last_action
        
        # Lógica especial para mantener DUCK por pájaros
        if userInput == "DUCK":
            self.duck_timer = max(self.duck_timer, 15)  # Mantener agachado por al menos 15 frames
        elif self.duck_timer > 0:
            self.duck_timer -= 1
            if self.duck_timer > 0:
                userInput = "DUCK"  # Forzar a mantener agachado
        
        # Resto del código original del método update...
        if self.dino_duck:
            self.duck()
        if self.dino_run:
            self.run()
