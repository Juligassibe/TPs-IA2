...
class AlgoritmoGenetico:
    ...
    def ejecutar(self) -> Dict[str, Any]:
        ...
        print(
            f"Generación 0: Mejor fitness = {self.mejor_fitness:.2f}, Fitness promedio = {np.mean(fitness_poblacion):.2f}")
...
def visualizar_comparacion_pasos(resultados_comparacion):
    ...
    print(
        f"Pasos totales (configuración original): {resultados_comparacion['resultados_original']['pasos_totales']:.2f}")
    ...
    print(
        f"Pasos promedio (configuración original): {resultados_comparacion['resultados_original']['pasos_promedio']:.2f}")
    ...
    for i, mejora in enumerate(resultados_comparacion["mejoras_ordenadas"][:5]):
        print(
            f"{i + 1}. Orden #{mejora['orden_num']}: Reducción de {mejora['pasos_original']:.2f} a {mejora['pasos_nueva']:.2f} pasos ({mejora['porcentaje_mejora']:.2f}% de mejora)")
    ...
    if empeoradas > 0:
        ...
        for i, empeora in enumerate(peores[:5]):
            print(
                f"{i + 1}. Orden #{empeora['orden_num']}: Aumento de {empeora['pasos_original']:.2f} a {empeora['pasos_nueva']:.2f} pasos ({abs(empeora['porcentaje_mejora']):.2f}% de empeoramiento)")
